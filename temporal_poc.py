...
def main():
    ...
    for img_num in range(NUM_IMAGES_TO_PARSE):
        for angle in range(0, len(cond_pose_images), NUM_FRAMES_PER_GEN):
            ...
            gen_images = pipe_I2I(
                prompt=[prompts[img_num]]*NUM_FRAMES_PER_GEN,
                negative_prompt=negative_prompts*NUM_FRAMES_PER_GEN,
                ...
                else [images[img_num]]*NUM_FRAMES_PER_GEN,
                generator=generator,
                height=HEIGHT,
                width=WIDTH,
            )

            generated_images_per_angle[img_num].append(gen_images.images)
            ...
            prompt_image = torch.permute(
                torch.tensor(
                    np.array(
                        (
                            paste_images_horizontally([im  for im in generated_images_per_angle[img_num][-2]])
                            if len(generated_images_per_angle[img_num]) > 1
                            else paste_images_horizontally([images[img_num]] * NUM_FRAMES_PER_GEN)
            ...
            pose_image = torch.permute(
                torch.tensor(
                    np.array(
                        paste_images_horizontally(control_images).resize(
